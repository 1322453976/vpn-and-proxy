...
add_to_prerouting()
{
    iptables -t nat -I PREROUTING -p tcp -j ${iptAccName}
    iptables -t mangle -I PREROUTING -p udp -j ${iptAccName}
}

del_prerouting_rule()
{
    iptables -t nat -D PREROUTING -p tcp -j ${iptAccName}
    iptables -t nat -X ${iptAccName}
    #
    iptables -t mangle -D PREROUTING -p udp -j ${iptAccName}
    iptables -t mangle -X ${iptAccName}
}
